...
# ── Sheet 2: Delta values per rat (matching heatmap b) ────────────────────────
...
# ── Sheet 3: Top 8 markers summary (matching panel d boxplots) ────────────────
...
# ── Sheet 4: Full LMM results (updated, clean labels) ─────────────────────────
lmm_full_sheet <- clean_markers(updated_results) %>%
  mutate(across(where(is.numeric), \(x) round(x, 4)))

# ── Sheet 5: Permutation test results (clean labels) ──────────────────────────
